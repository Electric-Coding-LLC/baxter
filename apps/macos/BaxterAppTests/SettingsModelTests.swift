...
final class RestoreBrowserIndexTests: XCTestCase {
    ...
    func testRestoreBrowserVisibleRowsCacheExpandsMultiPathDeltaFromTopLevelAncestor() {
    ...
    func testRestoreBrowserVisibleRowsCacheExpandsMultipleVisibleBranchesInOneTransition() {
        let roots = buildRestoreBrowserIndex(
            paths: [
                "/docs/notes/todo.txt",
        ...
        cache.resolve(
            ...
            rootPrefix: "",
            query: "",
            expandedPaths: ["/docs", "/music"],
            loadingDirectoryKeys: [],
            forceExpanded: false
        )
        XCTAssertEqual(cache.rows.map(\.id), ["/docs", "/docs/notes", "/music", "/music/albums"])
        ...
        cache.resolve(
            ...
            rootPrefix: "",
            query: "",
            expandedPaths: ["/docs", "/docs/notes", "/music", "/music/albums"],
            loadingDirectoryKeys: [],
            forceExpanded: false
        )
        ...
        XCTAssertEqual(
            cache.rows.map(\.id),
            ["/docs", "/docs/notes", "/docs/notes/todo.txt", "/music", "/music/albums", "/music/albums/song.mp3"]
        )
        XCTAssertEqual(
            cache.rows.filter(\.isExpanded).map(\.path),
            ["/docs", "/docs/notes", "/music", "/music/albums"]
        )
    }

    func testRestoreBrowserVisibleRowsCacheIgnoresHiddenExpandedPathChanges() {
        let roots = buildRestoreBrowserIndex(
            paths: [
                "/docs/notes/todo.txt",
                "/music/song.mp3",
        ...
        XCTAssertEqual(cache.rows.map(\.id), ["/docs", "/music"])
        ...
        cache.resolve(
            ...
            rootPrefix: "",
            query: "",
            expandedPaths: ["/docs/notes"],
            loadingDirectoryKeys: [],
            forceExpanded: false
        )

        XCTAssertEqual(cache.rows.map(\.id), ["/docs", "/music"])
        XCTAssertFalse(cache.rows.contains(where: { $0.path == "/docs/notes" }))
    }

    func testRestoreBrowserVisibleRowsCacheIncrementallyExpandsSinglePath() {
        let roots = buildRestoreBrowserIndex(
            paths: [
                "/docs/notes/todo.txt",
                "/music/song.mp3",
        ...
        XCTAssertEqual(
            cache.rows.map(\.id),
            ["/docs", "/docs/notes", "/docs/notes/todo.txt", "/music"]
        )
        XCTAssertEqual(cache.rows.filter(\.isExpanded).map(\.path), ["/docs", "/docs/notes"])
    }

    func testRestoreBrowserVisibleRowsCacheIncrementallyExpandsLoadingDirectory() {
        let roots = [
            RestoreBrowserNode(
                path: "/docs",
                name: "docs",
        ...
        XCTAssertTrue(cache.rows[0].isExpanded)
        XCTAssertTrue(cache.rows[0].isLoading)
        XCTAssertTrue(cache.rows[1].isLoadingPlaceholder)
    }
    ...
    func testRestoreBrowserVisibleRowsCacheIncrementallyCollapsesLoadingDirectory() {
        let roots = [
            RestoreBrowserNode(
                path: "/docs",
                name: "docs",
        ...
        XCTAssertEqual(cache.rows.map(\.id), ["/docs"])
        XCTAssertFalse(cache.rows[0].isExpanded)
        XCTAssertTrue(cache.rows[0].isLoading)
    }
    ...
    func testRestoreBrowserVisibleRowsCacheFallsBackToFullRebuildForAmbiguousExpansionChange() {
        let initialRoots = buildRestoreBrowserIndex(
            paths: ["/docs/notes/todo.txt"],
            maxPaths: 2_000
        ).rootNodes
        let replacementRoots = buildRestoreBrowserIndex(
            paths: ["/music/albums/song.mp3"],
            maxPaths: 2_000
        ).rootNodes
        ...
        cache.resolve(
            ...
            rootPrefix: "",
            query: "",
            expandedPaths: ["/music", "/music/albums"],
            loadingDirectoryKeys: [],
            forceExpanded: false
        )
        ...
        XCTAssertEqual(
            cache.rows.map(\.id),
            ["/music", "/music/albums", "/music/albums/song.mp3"]
        )
    }
